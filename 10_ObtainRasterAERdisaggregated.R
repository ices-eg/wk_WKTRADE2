...
### Plots ####
barplot1 <- ggplot(Values_perRegion_longformat, aes(sub_reg, value, fill=datasource))+geom_col(position = "dodge")+scale_y_continuous(labels = comma)+labs(x="subregion", y="total catch value [€]", title = " catch value")+theme(axis.title.x = element_blank(),axis.text.x = element_blank())
